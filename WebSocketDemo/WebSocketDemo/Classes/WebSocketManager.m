...
// 心跳定时器
@property(nonatomic, strong) NSTimer *headerBeatTimer;
// 没有网络的时候检测定时器
...
-(void)connectServer{
    ...
    self.webScoket = [[SRWebSocket alloc] initWithURL:[NSURL URLWithString:_linkURL]];
...
-(void)initHeartBeat{
    ...
    dispatch_main_async_safe(^{
        self.headerBeatTimer = [NSTimer timerWithTimeInterval:10 target:[WSWeakProxy weakProxyWithTarget:self] selector:@selector(senderheartBeat) userInfo:nil repeats:YES];
...
-(void)noNetWorkStartTestingTimer{
    __weak typeof(self)ws = self;
    dispatch_main_async_safe(^{
        ws.networkTestingTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:[WSWeakProxy weakProxyWithTarget:self] selector:@selector(noNetWorkStartTesting) userInfo:nil repeats:YES];
